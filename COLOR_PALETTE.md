...
| Old Testament / Tanakh refs (Genesis, Joshua, Isaiah, etc.) | Metallic emerald mini-pill | bg `linear-gradient #04321E → #15A86A → #04321E` + border `#2EFFA1` + text `#E6FFF2` | §3 expansion register — same treatment as the `<details>` expander buttons, scaled down |
| New Testament refs (Matthew, John, Romans, etc.) | Metallic gold mini-pill | bg `linear-gradient #645028 → #B4A078 → #645028` + border `#FCECAF` + text `#FFF8E1` | §1 priestly-witness register — deep / mid / deep sampled from the v4 brand-mark menorahs; border is the highlight specular |
| Extra-canonical refs (1 Enoch, Jubilees, Jasher, Apocrypha, Adam & Eve Conflict, Sonnini's Acts of Paul, etc.) | Metallic argaman mini-pill | bg `linear-gradient #3D1B5C → #8E4FB3 → #3D1B5C` + border `#D4B0E0` + text `#F5E6FA` | §1 covenant-body register — argaman extended to a metallic gradient; border is the §6 highlight palette Lilac as the bright companion tone |

**Pill treatment unified at S130 close.** All three source classes render with the same architectural treatment (deep-stop → midtone → deep-stop linear sweep + bright highlight border + near-white text in the register's color family). The deep-stop colors carry register-distinct hue while the visual structure stays uniform — readers learn one "metallic pill = cross-reference target" pattern and decode the source class by hue. Extras-tier targets render at 40% opacity when locked (route to `/pricing` on click); the argaman pill stays readable at reduced opacity so the source register signals through even on locked state.
