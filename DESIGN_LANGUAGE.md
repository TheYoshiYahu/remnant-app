...
> **S160 decision log — Gesenius dropped from V1.** §26 originally locked Gesenius (Tregelles 1846 English via OpenScriptures' AugIndex.xml) as the alternate Hebrew depth-lexicon, surfaced via the *Compare with Gesenius* toggle in the LexiconSheet header (Gate #2). The S160 first-pass Gesenius parse revealed `AugIndex.xml` is *only* an ordering-index (213 KB, 9,302 lines of `<w aug="N">lex-id</w>` entries mapping Tregelles 1846's print-edition entry-number sequence to LexicalIndex internal IDs) — it does NOT carry Gesenius body content. The parse-plan-fallback path (Tregelles 1846 PDF at archive.org → OCR → cleanup → Strong's alignment via AugIndex's print-page refs) is ~half a session of work with uncertain OCR quality and fragile alignment. Yoshi's S160 self-sufficient-application call: drop Gesenius from V1; ship Hebrew with BDB as the V1 single canonical source; reserve the second-Hebrew-lexicon slot for v1.1+ when a clean source is identified. Honors publish-then-edit; the framework callouts carry the depth that matters; one Hebrew source is honest and ships. Gate #2 below is updated inline; the *Compare with Gesenius* toggle, Compare-mode body layout, breadcrumb compare-toggle affordance, and `selectDefaultSource(language, available)` helper-API behavior all defer to v1.1+. The §VIII Provenance Log retains the AugIndex.xml entry as the surfacing artifact for the finding; the cloned source-data stays on disk for the future v1.1+ wheel that picks this up.

> **S160 decision log — BDB fidelity acceptable as-shipped for V1.** OpenScriptures BDB is abridged relative to the print edition — root analyses average 150–1,500 chars vs print's multi-page treatments on major terms like Elohim, Yahuah, dabar. S160 surfaced this as a partner-perceptible V1 fidelity question; Yoshi's call (delegated to Claude under the self-sufficient-application rule): ship V1 with OpenScriptures as-is. Three reasons. (1) the lexicon-body's job is to show how the inherited tradition handled the word — the framework callouts carry the depth that matters; abridged-but-faithful is the honest register. (2) BibleHub / BlueLetterBible richer-BDB scraping is fragile + license-questionable. (3) Publish-then-edit is the standing rule. A richer-BDB source investigation queues as a v1.1+ wheel if partner feedback flags the depth gap.

> **S160 decision log — §27 always-visible Strong's superscripts toggle.** New section locked S160 — BLB-pattern always-visible Strong's-numbers-on-every-word toggle in the chapter chrome strip + Settings reader-preferences. Free tier (no paywall — the Strong's data is already free at §20). Toggle chrome: metallic argaman pill (§9 extras-pill treatment, distinct from the §144 metallic-emerald *Hide English helpers* toggle). Superscripts: bracket-emerald midtone `#15A86A` (§3 expansion register). React implementation deferred to a future session per spec-then-build. Full spec at §27 below.
...
2. **Single canonical source per language (S160 update).** Hebrew words render BDB (Brown-Driver-Briggs 1906, OpenScriptures public-domain edition) as the V1 single canonical source. Greek words render LSJ (Tyndale-edited 1940, with Abbott-Smith 1922 NT-only fallback rendered seamlessly under the same LSJ source label). Aramaic words render BDB's Aramaic section (smaller than the Hebrew block but present). **No Compare toggle in V1** — the originally-locked *Compare with Gesenius* affordance defers to v1.1+ per the S160 Gesenius decision log above (Gesenius body content is not in the cloned OpenScriptures source; AugIndex.xml is an ordering-index, not body data; a Tregelles-1846-PDF-OCR pass is the deferred path). The single-source view ships at V1 — cleanest default, one source per word, room to breathe in the sheet. Sorting BDB primary on Hebrew is also the partner-recognition choice — BDB is the lexicon name most Hebrew-curious readers have heard, even if they've never used one. LSJ has lower partner-recognition than Thayer's in the typical Bible-app audience; mitigated by the standing-disclaimer block naming the lexicon clearly at the top of every entry. The second-Hebrew-lexicon slot (likely Gesenius, possibly a different public-domain dump if one surfaces) and the second-Greek-lexicon slot (Thayer's 1889 if sourced separately) both queue as v1.1+ alternate-source candidates — see *Deliberate non-prescription* below for both.
...
## 27. Always-Visible Strong's Superscripts Toggle (locked S160 — Free tier)

Companion surface to the §20 tap-on-word StrongsLookup modal. §20 hides the Strong's number until the partner taps a word; **§27 surfaces every Strong's number inline as a superscript next to every word in the verse**, when the partner has the toggle on. Pattern parity with Blue Letter Bible's `STRONG'S` header toggle — partners learning the original-language register can scan a chapter and see every H- and G-number without tapping each word in turn. Captured at S160 mid-session after Yoshi flagged the BLB pattern as a gap in the ROP reading surface.

The framework reading on why this is a Free-tier surface, not Companion-tier: the Strong's data ships with the verse payload regardless of tier (the §20 tap-on-word surface is already free; the data is on the page). Making the always-visible toggle Free-tier matches BLB's open-access posture for Strong's and keeps the awakening reader's drill-deeper path unobstructed. Companion-tier remains the home of the depth-lexicon bodies (BDB / LSJ / Gesenius per §26) — the *meaning* layer behind Strong's. §27 is the *visibility* layer over Strong's; depth stays gated, visibility stays open.

### Three load-bearing gates locked S160

1. **Tier gate = Free (no paywall).** Every partner — Free / Notes / Extras / Companion — sees the toggle in the chapter chrome strip and the Settings reader-preferences page, and can flip it on with no tier-locked chip and no `/pricing` redirect. The data is already free at §20; the visibility is also free here.

2. **Toggle chrome = metallic argaman pill (§1 covenant-body register, §9 metallic-argaman treatment).** Distinct from the §144 *Hide / Show English helpers* toggle (metallic emerald, §3 expansion register) so the partner reads the two chrome-strip controls as two different functional registers. The argaman gradient (`linear-gradient #3D1B5C → #8E4FB3 → #3D1B5C` + border `#D4B0E0` + text `#F5E6FA`) is the same treatment §9 locked for extras-tier cross-reference pills; extending it to the Strong's toggle is a UX-functional use of the register matching the precedent S130 set, not a theological-decorative one. Strong's superscripts surface the covenant-body language (Hebrew + Greek) under the English translation — the metallic argaman pointing at the covenant-body register is thematically resonant rather than arbitrary. Olive-leaf icy silver-blue at COLOR_PALETTE §4 was considered first and Yoshi corrected to metallic argaman at S160 mid-session; §4 stays *reserved* per the original lock.

3. **Superscript chrome = bracket-emerald midtone (§3 expansion register, `#15A86A`).** The inline superscripts themselves are rendered at ~70% of body-text size as `<sup>` elements after each word, in bracket-emerald midtone. Distinct from the §2 spectral-blue verse-numbers (which already render in superscript at chapter-start scale) — partners read two different superscript registers: spectral-blue for verse-anchors, bracket-emerald for Strong's-pointers. Tapping a Strong's superscript opens the §20 StrongsLookup modal for that word — the existing tap-the-word affordance stays unchanged; the superscript is a *second* tap target carrying the same modal-stack contract. Long-press on the word continues to open the §19 VerseActionMenu per the existing gesture lock.

### Defaultable surface

Items below ship with the noted V1 defaults; Yoshi redlines on §27 review at implementation-session close.

- **Toggle placement.** Two surfaces by Yoshi's S160 call:
  - **Chapter chrome strip — next to the S144 *Hide English helpers* toggle.** Same row, same chrome-button family, metallic-argaman gradient distinguishes it from the metallic-emerald *Hide English helpers* control. Tap toggles the visibility for the current reading session and persists the choice to `localStorage`. Label: `Strong's` (matching BLB's convention; concise on mobile chrome). When on, label shows the argaman-pill in its activated state; when off, label shows the same pill in its deactivated state per the existing `<details>`-button activated/deactivated pattern from §3.
  - **Settings → Reader preferences page.** Same toggle, same `localStorage` key, with a one-line description: *Show Strong's H- and G-numbers next to every word in the verse text.* Settings-page control is the canonical default-state surface; chrome-strip control is the moment-to-moment surface. Both write to the same persistence key so the partner sees consistent state across surfaces.
- **localStorage key.** `reader-prefs.strongs-superscripts` as boolean (matching the §144 `reader-prefs.english-helpers` naming convention). Default `false` — clean reading surface by default; the partner opts in to the superscripts.
- **Render position.** Superscript renders **after** the word, not before — matching BLB's convention and the way English readers scan left-to-right. Verse text reads naturally: `In the beginning H7225 God H0430 created H1254 the heaven H8064…` (Genesis 1:1 example). Word + superscript sit on the same baseline; the `<sup>` element CSS handles the size + raise.
- **Tap target on the superscript.** Tappable; opens the §20 StrongsLookup modal for that Strong's number. Hit target meets the §13 44pt iOS / 48dp Android floor where possible at the superscript scale; the word itself remains tap-target #1 (larger), the superscript is tap-target #2 (smaller but still touch-accessible per §13 reduced-size precedent on the per-word affordance). `aria-label="Strong's H7225 — open lexicon entry"` on each superscript for screen-reader access.
- **Coverage fallback — words without a Strong's number.** Some words in the verse text (English-only fillers like *the*, *and*, *of* when they don't carry a Hebrew/Greek backing word) have no Strong's number in `verse_words`. When the toggle is on, these words render with no superscript — same word, no chip. Partners see the gap visually and learn which words are translation-side English vs. covenant-body Hebrew/Greek without explanation needed.
- **Sacred-name body-text outline interaction.** The §1 techelet 4-direction outline on sacred names (*Yahuah*, *Yahusha*, *Elohim*) renders on the word *only*, not on the superscript. Superscript carries its own bracket-emerald color independent of the sacred-name register on the word it follows. No visual conflict; the two registers (techelet outline on word + bracket-emerald superscript after word) co-exist on the same line.
- **Hide-English-helpers interaction.** The two toggles are independent. A partner can show Strong's superscripts AND hide English helpers (`(God)` parentheticals after Hebrew names) — that's the *covenant-body register at its most surfaced* configuration: Hebrew names without English parentheticals + Strong's numbers next to every word. Or a partner can show both, hide both, or any combination. No cross-dependency.
- **Print / share interaction.** When the partner uses the §24 share-with-watermark surface to share a verse, the share-card renders with the partner's current Strong's-toggle state baked in — superscripts visible if the toggle was on at the moment of share. This honors the *what I see is what I share* principle from §24.
- **Chapter-end card interaction.** Strong's superscripts do NOT render in the chapter-end cross-reference card's verse-preview italic snippets. The card is a navigation surface, not a reading surface; superscripts there would compete with the cross-reference pills. The toggle's effect is scoped to the main verse text body only.

### Schema impact

None. The `verse_words` table already carries `strong_number` per word from Phase 5 (S120). The `strong_entries` table already carries the gloss + body. The §27 surface is a pure-frontend toggle over data that's already loaded. No migration needed; no API change needed.

### Implementation deferred to future session

Per the spec-then-build standard, §27's React implementation is NOT S160 work. S160 lands the spec; a future session opens the React work (chrome-strip toggle button, Settings-page entry, `<VerseText>` superscript rendering, localStorage hook). The implementation session will reference this section + the §20 / §144 prior patterns and ship via the helper-API + sanity-test surface convention from §22 / §23 / §24 / §25 / §26.

### What §27 deliberately does NOT prescribe

- **Strong's-data tier-gating.** Strong's data has been free since Phase 5 — §27 does not alter that. The toggle is visibility, not data-access.
- **Per-language toggle granularity.** No *show Strong's only for Hebrew* or *only for Greek* sub-toggle. One toggle, all-or-nothing across the verse text. A partner who wants per-language granularity is the rare advanced case; not V1 scope. Future-wheel candidate if partner-feedback flags it.
- **Per-POS toggle granularity.** No *show Strong's only for nouns* or *only for verbs* sub-toggle. Same rationale.
- **Inline gloss preview alongside the superscript.** No *show G976 (book) the book*-style inline gloss expansion. The superscript is the affordance; the gloss lives in the §20 modal one tap away. Inline-gloss-on-every-word would clutter the reading surface and competes directly with the body text. Permanently out of scope.
- **Strong's superscripts in commentary text.** Commentary body text (§17 chapter commentary) does not get Strong's superscripts. Commentary is the framework's voice; superscripting it would mix the proclamation with the apparatus and dilute the §17 *proclaimer-steps-aside* posture. Toggle scope is verse-text-only.
- **Strong's superscripts in lexicon-entry bodies (§26).** Lexicon-entry cross-references already render as `<a data-strong>` tap-links inside the lexicon body per §26; no additional superscript layer needed there.
- **Strong's superscripts on share-card watermarked verses' commentary surface.** Same as the commentary-text rule above — share-card watermarked verse is the proclamation surface; the apparatus stays scoped to the reading surface.
- **AI-generated *teach me this Strong's number* paraphrases.** Standing rule from §17 / §22 / §26 — no LLM-generated content in the partner-facing surface. The Strong's entry is the source; the framework callouts (in lexicon entries when the partner taps to a Companion-tier depth-lexicon) are author-written under voice-skill load.

---

## 28. Hebrew & Greek Interlinear Surface (locked S166, Phase 9.2 — Companion tier)

Closes the deferred Phase 9.2 deliverable that S158 punted behind 9.3 — with the depth lexicons (§26) live on production at S165, the interlinear opens at S166 as the next major reading wheel. §20 surfaces a single Strong's-tap modal per word. §27 surfaces every word's Strong's number as a superscript along the verse. §26 stacks the depth-lexicon body above the Strong's modal when the partner drills further. **§28 stacks the original-language depth — lemma, transliteration, morphology, gloss — as a column-layered view above every English word in the verse**, when the partner has the interlinear toggle on. The partner reads the verse word-by-word with the Hebrew or Greek visible above each English word, without leaving the chapter, without going to a credential-display tool elsewhere.

The framework reading on register: the interlinear is *come and see* applied to the original-language layer. The partner does not go to Blue Letter Bible, does not buy a parallel Bible, does not consult a lexicon back-and-forth between chapters. The work is on the page in front of them. The Hebrew or Greek behind every English word is visible at a glance; the parsing is in their own English; the gloss is the same gloss the Strong's modal carries. The interlinear is not a credential-display surface — it is the partner-facing visibility layer over the same Phase-5 verse_words foundation §20 / §27 / §26 all build on. The Pharisee-scholar posture would surface morph codes (`V-AAI-3S`) and lemma-only display and force the partner to consult a key; the framework posture surfaces the parsing in plain English abbreviation (`aor. act. ind. 3s`) and the lemma alongside its transliteration so the partner who has never opened a Hebrew grammar can still see what the original-language word IS.

### Four load-bearing gates locked S166

1. **Tier gate = Companion+ paywall on the toggle, the surface, and the morph-data API.** Aligns with the §20 *Locked stub catalog* (S122) which has carried `Hebrew/Greek interlinear` as Library-tier since the catalog was first locked — renamed to Companion at the S47 relock. The data lift behind the surface (per-verse morph + lemma + transliteration + gloss as a layered column above English) is the partner-facing apex of the original-language work; depth-lexicons (§26) and interlinear (§28) both live behind the Companion gate. Free / Notes / Extras partners see the chrome-strip toggle button rendered as tier-locked with the Companion-badge chip per the §20 tier-locked-stub contract; tapping routes to `/pricing`. Companion+ partners see the toggle live; tapping flips state and renders the interlinear layer. The morph + lemma + transliteration data is ALSO server-side tier-gated — below-Companion JWTs receive `morphology: null` from the chapter-words endpoint so the data doesn't ship to clients that aren't entitled. The Free-tier §20 Strong's tap surface remains unchanged; below-Companion partners keep the casual-reading depth (`strong_number` + short-gloss) the §20 modal already carries.

2. **Display register = above-verse stacked column, English-aligned word order.** Each English word in the verse has a stacked column rising above it. Top-to-bottom column order: **original-script lemma → transliteration → morphology (abbreviated) → gloss → English surface word**. The English surface word at the bottom is the existing verse-text word the partner is already reading; the interlinear toggle adds the layers above without replacing the verse-text register below. **Hebrew word order follows English flow** — the Hebrew lemma that sits behind each English word renders above that English word, in English-reading-order columns. The traditional RTL-Hebrew interlinear (where Hebrew runs right-to-left and the partner mirror-reads, with column ordering reversed) forces a reading-discipline register the come-and-see posture rules out: the partner is here to see the Hebrew word behind each English word, not to learn RTL-reading mechanics or to feel that the original-language layer is gated behind a literacy threshold. SBL Hebrew font renders each individual Hebrew lemma in its native RTL direction within its column-cell (so the Hebrew letters themselves read right-to-left within the lemma), but column-to-column order across the verse follows English. Greek is naturally LTR and aligns to English column order without bidi work. Aramaic blocks (Daniel 2:4b–7:28; Ezra 4:8–6:18, 7:12–26; Jeremiah 10:11) render in the same column-stack as Hebrew with the Aramaic morph register substituted; SBL Hebrew font covers Aramaic script.

3. **Morphology register = abbreviated English expansion, fully-written form on tap-and-hold.** STEPBible / Robinson Greek morph codes (`V-AAI-3S` = verb aorist active indicative 3rd singular) and Hebrew morph codes (`HVqp3ms` = Hebrew verb qal perfect 3rd masculine singular in the Westminster scheme) are credential-display register out of the box; partners new to original-language work bounce off them, and the Pharisee-scholar posture this work rules out is precisely *let the apparatus signal mastery the reader hasn't reached*. The locked path: render morphology as **abbreviated English expansion** — `aor. act. ind. 3s` for the Greek above, `qal pf. 3ms` for the Hebrew above. The abbreviated form decodes without a key for any partner who has read a Bible-app's grammatical-tag footnote even once; stays compact enough to fit the column width on mobile. **Tap-and-hold** on the morphology cell expands to **fully-written form** in a tooltip-style popover (`aorist active indicative, 3rd singular`; `qal perfect, 3rd masculine singular`). Same affordance pattern as the §26 lexicon cross-link long-press and the §22 NotesPanel long-press-to-expand. The abbreviated-and-on-hold-expanded register honors the come-and-see posture: the partner sees what the morphology is in their own English without leaving the chapter, and the full grammatical depth is one gesture away when wanted. No coded register surfaces in V1 at all — partners who want raw morph codes for export to a different study tool are the rare advanced case; not V1 scope.

4. **Toggle architecture = chapter chrome strip + Settings, parity with §27.** Toggle button in the chapter chrome strip, identical placement convention to §27's *Show Strong's* button (immediately adjacent — the two toggles read as a pair: §27 is *Strong's numbers on the verse text,* §28 is *original-language layer above the verse text,* both surfacing the covenant-body register at progressively deeper layers). Default OFF (clean reading surface, partner opts in). Persists to `localStorage` under `reader-prefs.interlinear` as boolean matching the §27 / §144 naming convention. Settings → Reader preferences page carries the same toggle as the canonical default-state surface. Companion-tier-only partners see the toggle live; below-Companion partners see the toggle rendered as tier-locked with the Companion-badge chip in the same chrome-strip pill register and route to `/pricing` on tap.

### Standing chrome treatment

The interlinear toggle pill uses the **metallic argaman** §27 register — same `linear-gradient #3D1B5C → #8E4FB3 → #3D1B5C` + border `#D4B0E0` + text `#F5E6FA` family that §27 carries. Continuity rationale: §27 and §28 both surface the covenant-body register (Hebrew + Greek under the English translation); they belong to the same chrome family. The Companion-badge chip on the §28 button for below-Companion partners distinguishes the depth-gate state from §27 (which has no chip — §27 is Free-tier). Two argaman pills in the chrome strip, one un-chipped (§27, Free), one Companion-chipped (§28, Companion); partners read the chrome strip as *Strong's surface (free), interlinear layer (Companion)*. Label on the toggle: `Interlinear`. When on, pill renders activated; when off, deactivated, per the existing chrome-button activated/deactivated pattern.

### Source + provenance

Same STEPBible / Tyndale House public-CC-BY-4.0 family that §26 lexicon entries draw from:

- **Hebrew + Aramaic — TAHOT** (Translators Amalgamated Hebrew Old Testament). Strong's-aligned per-word lemma + transliteration + morph + gloss covering every Hebrew + Aramaic word in the canonical OT. Sources OSHB (Open Scriptures Hebrew Bible) + alternate-alignment work + STEPBible editing. CC-BY-4.0; same provenance log entry pattern as TFLSJ from S159 §26.
- **Greek — TAGNT** (Translators Amalgamated Greek New Testament). Strong's-aligned per-word lemma + transliteration + morph + gloss across the NT, sourced from MAB / NA28 / SBLGNT amalgamation with STEPBible editing. CC-BY-4.0.

Both bundles join to existing `verse_words` rows on `(verse_id, position)` via the Strong's-number key already loaded at S120. The fetch path mirrors S159's STEPBible sparse-checkout (`source-texts/stepbible-data/` already cloned; add `TAGNT/` + `TAHOT/` directories to the sparse-checkout set). Attribution to Tyndale House Cambridge in `CONTENT_LICENSING.md` per the existing CC-BY pattern.

### Schema impact

**Single-column population, no ALTER.** `verse_words.morphology` already exists as a nullable TEXT column (S120 loader marked V2-deferred). The V1 §28 loader populates `morphology` from the TAHOT / TAGNT per-word morph tags. `lemma` and `transliteration` are joined at API-render time from `strong_entries` (the canonical lemma + transliteration are already loaded; per-instance inflected-form preservation is a v1.1+ enhancement when partner-feedback flags the gap on verbs whose inflected form differs partner-perceptibly from the lemma). No new tables; no ALTER TABLE; no migration filename. The loader script lands at `restoration-pipeline/_sessionNN_load_verse_words_morph.py` where NN is the implementation-session number.

**Tier-gating on the API.** The existing `GET /v1/books/:slug/chapters/:n/words` endpoint extends its response per word: when the JWT tier is Companion+, the row carries `morphology` (raw STEPBible code), `lemma` (from joined `strong_entries`), `transliteration` (from joined `strong_entries`), `gloss` (from joined `strong_entries.short_definition`). When the JWT tier is below Companion, those four fields render as `null` so below-Companion clients receive Strong's-number + surface only (the existing §20 contract preserved unchanged). Single endpoint, tier-aware payload.

### Helper API + sanity-test surface (the forward standard from §22 / §23 / §24 / §25 / §26)

Pure helpers in `app/src/lib/interlinear-helpers.ts` — no React imports, no global state, no async. Each function independently sanity-testable via `node --test` against inlined test cases per the prior-wheel pattern.

- `formatMorphology(rawCode, language, register='short')` — converts a STEPBible/Robinson raw morph code to the abbreviated English (`'short'`, e.g. `aor. act. ind. 3s`) or the fully-written register (`'expanded'`, e.g. `aorist active indicative, 3rd singular`). Hebrew and Greek code dictionaries inlined as constant maps. Sanity-test sweeps: each Greek POS family (V/N/A/Adv/Conj/Prep/Pron/Art/Particle) returns the right abbreviation across tense/voice/mood/person/number/gender/case axes / Hebrew binyanim (qal / niphal / piel / pual / hiphil / hophal / hitpael) + Aramaic peal/peil/pael return the right abbreviation / unknown raw code returns the raw code unchanged (defensive fallback) / null/empty raw code returns empty string / `'expanded'` register produces fully-written form for the same inputs / language-mismatch defensive (Hebrew code in Greek context returns raw).
- `selectInterlinearFontStack(language)` — returns the CSS `font-family` string for the lemma cell. `'hebrew'` / `'aramaic'` → `"'SBL Hebrew', 'Ezra SIL', 'Times New Roman', serif"`; `'greek'` → `"'SBL BibLit', 'Cardo', 'Times New Roman', serif"`; fallback returns the inherited body-text stack. Sanity-test sweeps: each language returns expected stack / unknown language returns body-text fallback / never returns empty string.
- `wrapHebrewBidi(lemma)` — wraps a Hebrew or Aramaic lemma string in Unicode RTL-isolation markers (U+2068 / U+2069) so the within-cell RTL rendering doesn't bleed into the surrounding LTR column-flow when the column ordering follows English. Sanity-test sweeps: Hebrew lemma with niqqud preserved within isolation / Hebrew with no niqqud preserved / Aramaic lemma isolated correctly / empty string returns empty string / non-RTL string passes through unchanged.
- `buildInterlinearColumn(verseWord, strongEntry, fontStack)` — assembles the per-word column-data structure `{ position, surface, lemma, transliteration, morphology: { short, expanded }, gloss, language, fontStack }` ready for render. Sanity-test sweeps: Hebrew word with full strong_entry + morph populates all fields / Greek word same / verse_word with `strong_number = null` returns surface-only column (English filler like *the*, *and* with no covenant-body backing) / strong_entry missing fields defensive / morph code empty defensive / column structure shape stable across language switches.
- `groupVerseColumns(verseWords, strongEntries)` — given the verse_words rows for a verse + the joined strong_entries map, returns the array of per-word interlinear columns in English flow order. Sanity-test sweeps: 5-word verse returns 5 columns in position order / mixed Strong's / non-Strong's words handled / empty verse returns empty array / out-of-order verse_words sorted by position / duplicate position defensive.

Verification target: ≥30 sanity-test cases passing in `_sNN_interlinear_sanity.mjs` at implementation-session close where NN is the loader-skeleton session number. tsc -b clean. No node sanity tests on the React InterlinearLayer component — that surface is stateful UI tested by live walk on the real PWA per the §22 / §23 / §24 / §25 / §26 pattern.

### Defaultable surface

Items below ship with the noted V1 defaults; Yoshi redlines on the §28 review.

- **InterlinearLayer component placement.** Rendered above each verse's existing English text when the toggle is on. The component is a child of the existing `<VerseText>` wrapper, not a sibling — it inherits the same verse-id context and shares the same right-pad / left-pad as the verse text below. Pattern parity with the §27 superscripts (which also sit inside `<VerseText>`).
- **Column-cell row order (top to bottom).** `lemma` (largest, original-script) → `transliteration` (smaller, italic, latin-script) → `morphology.short` (smallest, mono register) → `gloss` (smallest, italic register) → `surface` (the existing English verse-text word at the bottom; the layer above renders on top of the existing reading surface without replacing it). 1px `var(--reader-rule)` row-separator hairlines between layers when the toggle is on, suppressed in the layer-off state.
- **Column spacing.** Each column gets `min-width: 60px` on mobile, `min-width: 90px` on desktop; columns flow with `flex-wrap` so wide verses wrap to multiple lines. Wrapped lines maintain column-stacking discipline; no orphan cells.
- **Original-script font rendering.** SBL Hebrew (Hebrew + Aramaic) and SBL BibLit (Greek) loaded via `<link rel="preload" as="font" type="font/woff2" crossorigin>` at app boot — partners who toggle interlinear on don't see a font-fallback FOUT. Fonts hosted at the same Static Site as the rest of the PWA assets (no third-party CDN). The font-load is conditional on the Companion-tier JWT — below-Companion partners never see the preload (saves bandwidth + avoids loading fonts the partner can't reach).
- **Hebrew niqqud (Masoretic vowel-pointing) default.** TAHOT carries Masoretic-pointed text by default; V1 renders vowel-pointed Hebrew. The consonant-only register is a separate surface — the §20 stub catalog's W11 `Nikkudot siblings` item — and stays out of §28 scope.
- **Gloss cell content.** Renders `strong_entries.short_definition` (the single-phrase gloss already used by the §20 Strong's modal title). When a `lexicon_callouts` row exists for the Strong's number, the callout's `gloss_error_summary` does NOT override the gloss in the interlinear column — the column is the reference-data layer; the framework callout belongs in the §26 LexiconSheet where it can render the full corrective paragraph and quoted scripture. Surfacing the corrective summary in the column would compress the framework reading to a single phrase and risk the come-and-see posture (the reader doesn't see the work, only the conclusion). The gloss in the column is the lexicon's gloss as data; the LexiconSheet is where the framework reads the lexicon's gloss back.
- **Tap target on the column.** Tapping anywhere in the column opens the §20 StrongsLookup modal for that word — same as tapping the English word at the bottom of the column. Hit target meets the §13 44pt iOS / 48dp Android floor. The morphology cell additionally responds to tap-and-hold with the fully-written-form popover per the gate above.
- **Empty-state for verses without morph data.** Verses where `verse_words` carries Strong's but the TAHOT/TAGNT load hasn't populated morphology (early-chapter loading, scattered untagged verses, OT books with edge-case alignment gaps in the source data) render the column with `lemma` + `transliteration` + `gloss` rows visible and the `morphology` row blank-but-present (preserves vertical alignment across the verse). No empty-state copy in the cell; the absence reads as *this word has no morph data yet* without needing language. Coverage target: 100% of canon for V1 (TAHOT + TAGNT cover the full Protestant canon); partial-coverage degradation handled gracefully.
- **Verses entirely without verse_words rows.** Extras-tier books (Apocrypha, 1 Enoch, Jubilees, Jasher, etc.) don't have verse_words at V1 — they were never Strong's-tagged. With the interlinear toggle on, extras-book verses render the English verse text as-is with no interlinear layer above. No empty-state message; the absence reads as *interlinear not available for this book*. A future v1.1+ wheel could backfill verse_words for select extras texts where Strong's-aligned manuscripts exist (1 Enoch's Ge'ez vs Aramaic fragments — speculative); not V1 scope.
- **Persistence.** `reader-prefs.interlinear` boolean in localStorage. Toggle state persists across chapters, sessions, app restarts. Default `false`.
- **Loading state.** First verse render after toggling on doesn't show a spinner — the chapter-words API call already cached the per-word data at chapter-load time per the existing §20 pattern; toggling interlinear is a pure-frontend layer over data already in memory. The Companion-tier JWT entitlement check happens at toggle-on time — partners who upgrade mid-chapter need to refresh to pick up the new entitlement (standard PWA-tier-refresh pattern; no V1 work to dynamically re-fetch on tier-change mid-session).
- **Interaction with §27 (Strong's superscripts).** Both toggles independent. A partner can show interlinear AND show Strong's superscripts simultaneously — the superscript renders on the English surface word at the bottom of each interlinear column, just as it renders without the interlinear layer. The lemma cell at the top of the column carries the Strong's number nowhere on its own; the column-as-a-whole is the §28 affordance, the superscript-on-the-English-word is the §27 affordance, and the two read as complementary layers of the same data source.
- **Interaction with §144 (Hide English helpers).** Both independent. A partner can show interlinear AND hide the `(LORD)` / `(God)` parentheticals on sacred names — the English surface word at the bottom of the column carries the parenthetical or not per the §144 toggle state; the lemma cell at the top renders the Hebrew sacred name (Yahuah, Elohim, etc.) in original script without parenthetical regardless.
- **Interaction with §1 sacred-name techelet outline.** The techelet 4-direction outline renders on the English surface word at the bottom of the column (where it already renders when the interlinear is off) AND on the lemma cell at the top when the lemma is itself a sacred name (Yahuah, Yahusha, Elohim, etc.). Two layers of the same name carry the same outline register; the partner reads the covenant-body register coherently across the column.

### Accessibility

- **WCAG 2.1 AA per §13.** Each interlinear column is a `<div role="group" aria-label="Word {position}: {transliteration_or_surface}">` so screen-reader users navigate column-by-column. The lemma cell carries `lang="he"` / `lang="el"` / `lang="arc"` (Aramaic) for screen-reader pronunciation hint where the OS speech engine supports it. The transliteration cell carries `lang="en"` and the morphology + gloss cells inherit the document language.
- **Reading order.** Each column reads linearly top-to-bottom (lemma → transliteration → morphology → gloss → surface), then next column. The DOM order matches the visual column order so screen readers traverse correctly without `aria-flowto` complexity.
- **Tab order.** Each column is keyboard-focusable as a single unit (`tabindex="0"` on the column wrapper); within a column, the morphology cell is the only interactive child (long-press surfaces the expanded form, which is also reachable via keyboard `Enter` on the cell). Tab order across the verse: chapter chrome toggle → verse 1 column 1 → verse 1 column 2 → ... → verse 1 column N → verse 2 column 1 → ...
- **Reduced motion.** Toggle on/off is instant (no slide-up animation, no fade). Long-press popover on morphology expansion respects `prefers-reduced-motion: reduce` (instant show/hide rather than scaled fade).
- **High-contrast mode.** Column row-separator hairlines (`var(--reader-rule)`) meet WCAG 2.1 AA contrast against `var(--reader-bg)` in both light and dark themes per the §1 theme tokens.
- **Hit targets.** Column tap targets always meet the §13 44pt iOS / 48dp Android floor. Morphology long-press hit-target is the same column area; the long-press distinguishes from the tap, not a separate target.
...
- **Multiple original-text variant comparison.** No Westcott-Hort vs Stephanus vs Byzantine Greek toggle; no Aleppo vs Leningrad Hebrew toggle. One canonical source per language at V1 — STEPBible TAGNT for Greek, TAHOT for Hebrew. Cross-variant comparison is a translator's surface (the same scope-exclusion §26 made for lexicon-edition comparison); defers to v1.1+ only if partner-feedback flags demand.
- **Consonant-only Hebrew toggle (vowel-pointing removal).** V1 ships Masoretic vowel-pointed Hebrew per the TAHOT default. The consonantal-form register is the §20 stub catalog's W11 *Nikkudot siblings* item — separate surface, separate wheel. Adding a consonant-only toggle inside §28 would conflate two distinct affordances.
- **AI-generated translation alternatives.** Standing rule from §17 / §22 / §26 / §27 — no LLM-generated content in the partner-facing surface. This rules out *but you could also translate this as…* hover-popovers, *modernize this word into contemporary English* expansions, and *here's how a different translation renders this* alternate-gloss displays. The gloss is the lexicon's gloss; the framework's reading lives in commentary and in §26 callouts, not in the column-stack.
- **Audio narration of original-script.** §25 TTS reads English scripture; reading Hebrew or Greek aloud at the rendering layer mixes proclamation registers and would require pronunciation models the framework hasn't sanctioned. The partner listens to English; the interlinear is silent visual depth.
- **Per-partner annotation on interlinear columns.** Partners cannot note-anchor on an interlinear column. Notes anchor to verses, not to lexicon entries (§26 rule), not to interlinear columns (§28 rule). A partner who wants to record a thought about a specific word's morphology writes it on the verse where the word surfaced via the §22 NotesPanel, with the verse context attached. This keeps the notes surface anchored to scripture.
- **Strong's superscripts inside the interlinear column.** The lemma + transliteration + morphology + gloss carries the data the superscript points to; doubling the Strong's number inside the column-stack would clutter the reading register. §27's superscript renders on the English surface word at the bottom of the column when the §27 toggle is independently on; the lemma cell at the top of the column carries the original-script word, not a Strong's number repetition.
- **Interlinear in chapter-end card verse-preview snippets.** Same scope as §27's exclusion — cards are navigation, not reading. The verse-preview italic snippet in a cross-reference card renders English-only regardless of the partner's interlinear-toggle state.
- **Interlinear in §24 share-with-watermark.** Same rule — the share-card is the proclamation surface; the apparatus stays scoped to the reading surface. A partner who wants to share what an original-language word looks like screenshots the InterlinearLayer themselves via OS-level screenshot, with the column visible in the image so the source is named — that's their call, not a §28 affordance.
- **Interlinear in §17 chapter commentary text.** Commentary is the framework's voice; interlineated commentary would mix proclamation with apparatus and dilute the *proclaimer-steps-aside* posture. Toggle scope is verse-text-only.
- **Interlinear inside the §26 LexiconSheet body.** The lexicon's own derivation-chain and cross-link affordances handle within-entry navigation; layering an interlinear-style column-stack inside a lexicon entry would conflate two reference registers. The LexiconSheet stays a single-source depth surface; the interlinear stays a verse-text overlay surface.
- **Per-instance lemma vs canonical lemma split.** V1 ships canonical-lemma rendering (the `strong_entries.lemma` value, same lemma the §20 Strong's modal title carries). Per-instance inflected-form preservation — where the actual surface form differs partner-perceptibly from the lemma (Greek verb conjugations, Hebrew construct chains, etc.) — defers to v1.1+ if partner-feedback flags the gap. Adding per-instance lemma at V1 inflates schema cost (new column or new table) and ships a partner-perceptibly-marginal improvement for the common case where canonical lemma is what the partner wants to see.
- **Per-instance transliteration distinct from canonical transliteration.** Same scope-exclusion as the per-instance lemma above. V1 ships canonical transliteration from `strong_entries.transliteration`; per-instance inflected-form transliteration defers.
