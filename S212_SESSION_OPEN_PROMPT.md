...
- S211 NOT YET COMMITTED (Yoshi commits from his Mac): verse-pills strip moved
  below the cards (order now xref → Witness → Kingdom → pills → commentary);
  ArrangedReading overlay + `chronological-reading.json`. Commit:
  `git add app/src/App.tsx app/src/components/ArrangedReading.tsx app/src/data/chronological-reading.json && git commit -m "..." && git push`
