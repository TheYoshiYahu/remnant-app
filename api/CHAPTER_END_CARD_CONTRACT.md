...
**Tier-locked rows (S140 — Option C policy).** The S73 "greyed-out item with a small lock affordance" render policy is retired. Two surfaces, two treatments:

- **Baseline (per-verse cross-reference target).** A locked target verse renders at reduced opacity (≈40%) with an "Unlock in [Name] tier" tooltip. Click routes the partner to `/pricing` instead of the target verse. The reader can see the source-verse group and the target-verse pill exists, but can't follow the link or read the preview at full visual weight.
- **Threads (Layer 2 callout).** A locked thread renders title + anchor + a ~70-word teaser pulled from the first paragraph of `summary_md`, followed by a `linear-gradient(transparent → surface)` fade over the bottom ~56px of the teaser block. Below the fade: a single line "Full summary + N verse pairings in this chapter" (where N is the count of `members_in_chapter`), then a primary CTA reading "Unlock in [Name] tier." Members do NOT render. No per-row lock icons, no expand button, no greyed-out body. The free reader is pulled into the start of a framework-bearing reading and stopped at the gate where the prose gets substantive.
