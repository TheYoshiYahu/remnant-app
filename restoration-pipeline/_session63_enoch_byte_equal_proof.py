#!/usr/bin/env python3
"""
_session63_enoch_byte_equal_proof.py
------------------------------------
Runs the S63 Enoch sync (in-memory) against the current .txt, re-parses
the result via parse_enoch, and diffs the resulting JSON against the
live parsed/enoch.json.
...
SYNC_PATH = os.path.join(APP, "restoration-pipeline", "_session63_sync_enoch_to_txt.py")
spec = importlib.util.spec_from_file_location("s63_sync", SYNC_PATH)
...
def main():
    ...
    # Chapter-level summary: how many chapters still diverge and on what
