...
def url():
    v=[l for l in Path('/tmp/repair2-wt/api/.env').read_text().splitlines() if l.startswith('DATABASE_URL=')][0].split('=',1)[1].strip().strip('"').strip("'")
...
async def main():
    ...
    # BACKUP
    Path('/tmp/repair2-wt/scratch_repair/backup').mkdir(parents=True,exist_ok=True)
    ...
    bpath=f"/tmp/repair2-wt/scratch_repair/backup/book{BOOK_ID}_{int(time.time())}.json"
