...
Also prints the live Revelation thread count.
...
async def main():
    ...
    n=await conn.fetchval("""
      ...
      WHERE e.slug='canon' AND b.slug='2-corinthians'""")
    m=await conn.fetchval("""
      ...
      JOIN editions e ON b.edition_id=e.id
      WHERE e.slug='canon' AND b.slug='2-corinthians'""")
    print(f"LIVE 1-Corinthians threads: {n}  members: {m}")
    await conn.close()
    # check fragments
    total_bad=0
    for frag in frags:
