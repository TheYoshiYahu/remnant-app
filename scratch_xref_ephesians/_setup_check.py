...
async def main():
    ...
    # 2. ephesians-* threads (greenfield check)
    heb=[s for s in slugs if s.startswith("ephesians-")]
    print(f"ephesians-* threads in live DB: {len(heb)}")
    if heb: print("  ", heb[:10])
    ...
    chs=await conn.fetchval("""SELECT count(*) FROM chapters c JOIN books b ON c.book_id=b.id
      JOIN editions e ON b.edition_id=e.id WHERE e.slug='canon' AND b.slug='ephesians'""")
