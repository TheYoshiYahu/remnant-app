...
async def main():
    ...
    # 2. galatians-* threads (greenfield check)
    heb=[s for s in slugs if s.startswith("galatians-")]
    print(f"galatians-* threads in live DB: {len(heb)}")
    if heb: print("  ", heb[:10])
    # 3. galatians anchored threads via book join (any slug)
