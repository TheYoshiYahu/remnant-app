...
Also prints the live Galatians thread count.
...
async def main():
    ...
    n=await conn.fetchval("""
      ...
      WHERE e.slug='canon' AND b.slug='galatians'""")
    m=await conn.fetchval("""
      ...
      JOIN editions e ON b.edition_id=e.id
      WHERE e.slug='canon' AND b.slug='galatians'""")
    print(f"LIVE Galatians threads: {n}  members: {m}")
    await conn.close()
    # check fragments
    total_bad=0
    for frag in frags:
