...
async def main():
    ...
    # 2. thessalonians-* threads (greenfield check)
    heb=[s for s in slugs if s.startswith("thessalonians-")]
    print(f"thessalonians-* threads in live DB: {len(heb)}")
    if heb: print("  ", heb[:10])
    ...
    n=await conn.fetchval("""SELECT count(*) FROM cross_reference_threads t
      ...
      WHERE e.slug='canon' AND b.slug='thessalonians'""")
    ...
    chs=await conn.fetchval("""SELECT count(*) FROM chapters c JOIN books b ON c.book_id=b.id
      JOIN editions e ON b.edition_id=e.id WHERE e.slug='canon' AND b.slug='thessalonians'""")
