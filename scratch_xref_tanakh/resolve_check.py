#!/usr/bin/env python3
"""Authoritative resolution check against PROD DB.
For each input tuple in the given fragments, confirm BOTH the source ref and the
target ref resolve to a real verse in the live DB (exactly what the migration's
inner JOINs require). Any ref that does NOT resolve = a silently-dropped row.
Also prints the live Acts thread count.

Usage: .venv/bin/python resolve_check.py <frag1.sql> [frag2.sql ...]
"""
        ...
        for idx,ch in enumerate(region):
            if ch=='(' and depth==0: depth=1;start=idx+1
                ...
                if depth==0 and start is not None:
                    ...
                    # input cols: src_ed,src_slug,src_ch,src_v,tgt_ed,tgt_slug,tgt_ch,tgt_v,tier,note
...
async def main():
    frags=sys.argv[1:]
    url=load_db_url()
    if 'sslmode' not in url: ssl='require'
    else: ssl=None
    ...
    # live thread count for the book under build (BOOK_SLUG env, default 'acts')
    book=os.environ.get("BOOK_SLUG","acts")
    n=await conn.fetchval("""
      ...
      WHERE e.slug='canon' AND b.slug=$1""", book)
    m=await conn.fetchval("""
      ...
      JOIN editions e ON b.edition_id=e.id
      WHERE e.slug='canon' AND b.slug=$1""", book)
    print(f"LIVE {book} threads: {n}  members: {m}")
    await conn.close()
    # check fragments
    total_bad=0
    for frag in frags:
